//! This module is a barely-modified copy of
//! the config setup in the server. Until it's
//! stable it won't be public.
...
#[derive(Clone, Debug)]
pub struct PlaneInfos(pub HashMap<PlaneType, PlaneInfo>);
#[derive(Clone, Debug)]
...
impl Index<PlaneType> for PlaneInfos {
    type Output = PlaneInfo;

    fn index(&self, idx: PlaneType) -> &PlaneInfo {
        &self.0[&idx]
    }
}

impl Index<MobType> for MobInfos {
    type Output = MobInfo;

    fn index(&self, idx: MobType) -> &MobInfo {
        &self.0[&idx]
    }
}
